...
class MonteCarlo:
    ...
    def populate_levels(self):
        ...
        energy_array = [
            num_up * self.Energy_levels[1]
            + (self.total - num_up) * self.Energy_levels[0]
        ]
        for i in track(
            range(self.total),
            description="[blue] Determining populations of energy levels:",
        ):
...
def S(total, up):
    ...
    if up * down == 0:
        return 0
    elif stirling_only or (
        overflow_ratio is not None
        and percent_up > overflow_ratio
        and percent_up <= 1 - overflow_ratio
    ):
...
def P_curve(T_max, Energy_levels, allow_negative=False):
    ...
    p2_array = []
    for T in track(
        T_array, description="[blue] Calculating probabilities vs temperature:"
    ):
        try:
...
def getArgs():
    ...
    parser.add_argument(
        ...
        default=10000,
        type=int,
        help="number of particles in the system. Default is 10000.",
    )
    parser.add_argument(
        "-t", "--temp", default=25, help="temperature of the system. Default is 25 K."
    )
    parser.add_argument(
        "-e",
        "--energy",
        nargs=2,
        default=[0, 6],
        help="energy levels of the system in multiples of kb. Default is 0 kb*K and 6 kb*K.",
    )
    parser.add_argument(
        "-l",
        "--starting_level",
        type=int,
        default=0,
        help="starting level of the system. Either 0 or 1. Default is 0.",
    )
    parser.add_argument(
        "-m",
        "--monte",
        default=False,
        action="store_true",
        help="run the monte carlo simulation",
    )
    parser.add_argument(
        "-p",
        "--prob",
        default=False,
        action="store_true",
        help="plot the probability curves",
    )
    parser.add_argument(
        "-q",
        "--quiet",
        default=False,
        action="store_true",
        help="don't plot the S(E) curve for the monte carlo simulation.",
    )
    parser.add_argument(
        "-s",
        "--se_curve",
        default=False,
        action="store_true",
        help="plot the theoretical S(E) curve.",
    )
    parser.add_argument(
        "-x",
        "--exact_omega",
        default=False,
        action="store_true",
        help="Calculate the exact value of omega via combinatorics until overflow errors force a switch to stirling. If excluded, the program will only use the stirling approximation.",
    )
...
if args.monte:
    ...
    print(
        f"The number of particles in the upper level is {sum(system.starting_array)} out of {total} particles."
    )
    print(
        f"This ratio is {sum(system.starting_array)/total:.4f}, compared to the expected ratio of {system.probs[1]:.4f}."
    )
    ...
    print(
        f"The numerically determined temperature is {final_temp:.2f} K, compared to the expected temperature of {Max_T} K."
    )
    ...
    if not args.quiet:
        ...
        plt.xlabel("Energy (J)", fontsize=20)
        plt.ylabel("Entropy (J/K)", fontsize=20)
        plt.title(f"System with {total} particles", fontsize=20)
...
if args.prob:
    ...
    plt.title(f"Probability of finding a particle in each level", fontsize=20)
...
if args.se_curve:
    ...
    fig.suptitle(f"System with {total} particles", fontsize=20)

    ax2.plot(E_array[1:], slope_array)
    ax2.set_ylabel("dS/dE (1/K)", fontsize=20)
    ax2.tick_params(axis="both", which="major", labelsize=20)

    ax3.plot(E_array[1:], temp_array)
    ax3.set_ylabel("T (K)", fontsize=20)
    ...
    ax1.set_xlabel("E (J)", fontsize=20)
    ax1.set_ylabel("S (J/K)", fontsize=20)
